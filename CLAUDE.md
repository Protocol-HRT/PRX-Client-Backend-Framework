...
- **React frontend repo** — working frontend lives at `/var/www/html/atlas-dev-local` (Next.js theme port, separate from this repo), consumes `/api/v1/`. The old `docs/theme-landing-page-in-ts/` boilerplate reference has been removed. Frontend branding (logos, titles, copy) is being migrated to `/api/v1/config` + CMS — no per-client branding may remain hardcoded in either codebase.
